...
-- 3번 고객이 주문한 도서의 최고 금액보다 더 비싼 도서를 구입한 다른 주문의 주문번호, 금액 표시
SELECT *
  FROM Customer;
...
-- SELECT절 서브쿼리, JOIN으로 변경가능 이미 쿼리가 너무 복잡해서 더이상 테이블을 추가하기 힘들면,
-- 많이 사용
-- 고객별 판매액을 보이시오 GROUP BY가 들ㅇ가면 SELECT 절에 반드시 집계함수가 들어가야 함!!!
SELECT SUM(o.saleprice) AS '고객별 판매액'
 ...
 -- 업데이트, 한꺼번에 필요한 필드값을 한테이블에서 다른테이블로 복사할 때 아주 유용함.
 UPDATE Orders
